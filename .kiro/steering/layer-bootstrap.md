...
- `@ComponentScan(basePackages = "com.awsome.shop.point")` 扫描所有模块
- `@EnableScheduling` 启用定时任务（积分自动发放）
- 聚合所有 impl 模块的 Maven 依赖，由 Spring DI 完成接口与实现的绑定
- 配置类（`config/` 包下）：`OpenApiConfig`（Swagger）、`AsyncConfig`
...
- 当前表：point_balances、point_transactions、system_configs
- 设计文档要求新增：distribution_batches（发放批次表）
