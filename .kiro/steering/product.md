...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/v1/point/balance | 查询当前用户积分余额 |
| GET | /api/v1/point/transactions?page=&size= | 查询当前用户积分变动历史 |
...
| 方法 | 路径 | 调用方 | 说明 |
|------|------|--------|------|
| POST | /api/v1/internal/point/init | auth-service | 初始化用户积分余额 |
| POST | /api/v1/internal/point/deduct | order-service | 兑换扣除积分 |
| POST | /api/v1/internal/point/rollback | order-service | 回滚积分扣除 |
| GET | /api/v1/internal/point/balance/{userId} | order-service | 查询指定用户积分余额 |
...
| BAD_REQUEST_001 | 400 | 扣除后余额不足 | 管理员手动扣除时余额不足 |
| BAD_REQUEST_002 | 400 | 积分不足，无法兑换 | 兑换扣除时余额不足 |
...
- page: 默认 0，≥ 0（设计文档规范）
- size: 默认 20，范围 1 ~ 100
- 注意：MyBatis-Plus 分页从 1 开始，实现层需要 page + 1 转换
...
## 跨服务交互
- auth-service → POST /api/v1/internal/point/init（注册成功后，失败不影响注册）
- order-service → POST /api/v1/internal/point/deduct / rollback，GET /api/v1/internal/point/balance/{userId}
