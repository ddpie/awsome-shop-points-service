...
API 路径以设计文档（aidlc-docs/construction/points-service）定义为准，使用标准 RESTful 风格。

### 3.1 员工端点（/api/points）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/points/balance | 查询当前用户积分余额（X-User-Id 请求头） |
| GET | /api/points/transactions?page=&size=&type= | 查询当前用户积分变动历史 |

### 3.2 管理员端点（/api/admin/points）
...
### 3.3 内部端点（/api/internal/points，服务间调用）

| 方法 | 路径 | 调用方 |
|------|------|--------|
| POST | /api/internal/points/init | auth-service |
| POST | /api/internal/points/deduct | order-service |
| POST | /api/internal/points/rollback | order-service |
| GET | /api/internal/points/balance/{userId} | order-service |
...
| INSUFFICIENT_BALANCE_ADJUST | BIZ_001 | 200 | 扣除后余额不足 | POINTS_002 |
| INSUFFICIENT_BALANCE_REDEEM | BIZ_002 | 200 | 积分不足，无法兑换 | POINTS_003 |
| TRANSACTION_NOT_FOUND | NOT_FOUND_002 | 404 | 积分变动记录不存在 | POINTS_004 |
| INVALID_ROLLBACK_TYPE | BIZ_003 | 200 | 只能回滚兑换扣除记录 | POINTS_005 |
| DUPLICATE_ROLLBACK | CONFLICT_001 | 409 | 该笔扣除已回滚，不可重复操作 | POINTS_006 |
| CONFIG_NOT_FOUND | NOT_FOUND_003 | 404 | 配置项不存在 | POINTS_007 |

说明：设计文档使用 `POINTS_xxx` 前缀，实际实现使用 `{CATEGORY}_{SEQ}` 格式以配合 GlobalExceptionHandler 的 HTTP 状态码自动映射。

---

## 六、并发控制

- 所有余额变动操作使用悲观锁（`SELECT ... FOR UPDATE`）— BR-004
- 适用场景：兑换扣除、管理员调整、积分回滚、定时发放
- PointBalanceMapper 提供 `selectByUserIdForUpdate` 方法
...
| getBalance | 2 | 正常返回 / 不存在抛异常 | POINTS_001 |
...
| distributeToUser | 3 | 发放成功(DISTRIBUTION) / 余额不存在跳过 / 余额更新与流水同时创建 | BR-003, BR-008, BR-010 |
...
| BR-004 | 悲观锁并发控制 | adjustPoints/deductPoints/rollbackDeduction 均使用 getByUserIdForUpdate |
